...
export function createCodeqExtension({
    ...
    return async function execute(_toolCallId, params, signal, onUpdate, ctx) {
      try {
        ...
        if (
          ...
          request.context = fullGrepContext;
        }
        ...
        const formatted = format(name, ranked, {
          detail: params?.detail === "full" ? "full" : "summary",
        });
        return {
  ...
  const tools = [
    ...
    },
    {
      ...
      promptGuidelines: [
        "grep is codeq content search, not rg. One identifier or one regex per call (regex is auto-detected). All-match patterns like .* are rejected.",
        ...
        "grep searches one repository per call. Pass root for another checkout; pass path to narrow inside the selected root. After 1-2 greps, read the named span. Use graph for the next hop in a call chain.",
      ],
      parameters: Type.Object({
        pattern: Type.String({
          description:
    ...
    },
    {
      ...
      promptGuidelines: [
        ...
        "graph returns the hit symbol's own span and its direct callees. Read that span; do not split a pipeline into one graph call per identifier. There is no callers tool.",
        'graph searches one repository per call. Pass root for another checkout; pass path to narrow. Default is layer 0 (no source). Pass detail: "full" only to quote code.',
      ],
      parameters: Type.Object({
        query: Type.String({
          description:
