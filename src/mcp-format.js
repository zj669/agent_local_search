...
- graph: how code works, a symbol, callers, callees, impact, or "where is X used". Also use graph for call chains and pipelines (for example tracing which functions a request passes through from an entry handler): once you know one entry symbol from a single grep, query graph with identifiers or "how does X work" where X is that symbol — not a multi-paragraph question. graph returns direct callees with file:line for each hop; prefer it over Bash or repeated grep for the next hop. One call is enough — explore already includes related files, call paths, and blast radius. There is no callers tool. graph returns a map of the code to read next, not a written answer.
